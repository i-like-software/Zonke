...
function runPlannedEngine(budget: number): {
  ...
  // Step 2: pay overdue first (urgent)
  ...
  // Step 3: dump leftover onto highest interest (avalanche)
  if (remaining > 0) {
    const sorted = [...allocs]
  ...
  // Step 4: label reasons & recalculate payoff
  allocs.forEach(a => {
    ...
    // months to payoff with this payment
...
function runStressEngine(budget: number): {
  ...
  // Priority: 1=overdue, 2=due≤3days, 3=due≤7days, 4=rest by interest
  ...
  return { advice, covered, shortfall: Math.max(0, totalMin - covered), canCoverAll, tip };
}
...
function useTypewriter(text: string, speed = 12, active = true) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      idx.current += 3; // advance 3 chars per tick for speed
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-slate-50">
              {result.allocations.map((alloc, i) => {
                const pct = alloc.account.balance > 0
                  ? Math.round((alloc.payment / alloc.account.balance) * 100) : 0;
                return (
                    ...
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          {alloc.account.status === "overdue" && (
                            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-red-100 text-red-600">
                              OVERDUE
                            </span>
                          )}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-[10px] text-slate-400">
                          {alloc.monthsToPayoff < 999 ? `~${alloc.monthsToPayoff}mo to clear` : "long-term"}
                        </p>
                      </div>
                    </div>
                    {/* Payment progress on balance */}
                    <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-700"
                        style={{
                          width: `${pct}%`,
                          background: alloc.account.status === "overdue" ? "#E8253A" : "oklch(0.62 0.13 75)",
                          transitionDelay: `${i * 100}ms`,
                        }}
                      />
                    </div>
                    <p className="text-[10px] text-slate-400 mt-1">
                      {fmt(alloc.payment)} of {fmt(alloc.account.balance)} balance ({pct}%)
                    </p>
                  </div>
                );
              })}
            ...
            {result.leftover > 0 && (
              <div className="px-5 py-3 bg-emerald-50 border-t border-emerald-100 flex items-center gap-2">
                ...
                <p className="text-xs text-emerald-700">
                  <strong>{fmt(result.leftover)}</strong> unallocated, consider putting it toward Truworths to clear faster.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-100 p-4 flex gap-3">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              <strong>Avalanche method:</strong> Always pay minimums on all accounts first,
              then throw every extra rand at the highest interest rate account. This minimises
      ...
      {show && (
        <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          ...
                          {item.amount > 0 && (
                            <p className="text-sm font-bold text-slate-900 shrink-0">{fmt(item.amount)}</p>
                          )}
                        </div>
            ...
            <div className="bg-amber-50 rounded-2xl border border-amber-100 p-4 flex gap-3">
              ...
              <div>
                <p className="text-xs font-semibold text-amber-800 mb-1">
                  {fmt(result.shortfall)} shortfall this month
                </p>
                <p className="text-xs text-amber-700 leading-relaxed">
                  Call any skipped stores and ask for a payment arrangement, SA retailers have hardship
                  programmes. It won't hurt your credit score if you communicate proactively.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-100 p-4 flex gap-3">
            <ShieldCheck className="w-4 h-4 text-slate-400 shrink-0 mt-0.5" />
            <p className="text-xs text-slate-500 leading-relaxed">
              <strong>Remember:</strong> {result.tip}
            </p>
          </div>
        </div>
      )}
    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] max-w-sm mx-auto text-center gap-6">
      <div>
        ...
        <p className="text-sm text-slate-500 leading-relaxed">
          Tough month? Enter what you can afford and we'll tell you exactly what to pay,
          what to skip, and how to protect your credit score.
        </p>
      </div>
      ...
      <div className="w-full">
        ...
        {val > 0 && val < totalMin && (
          <p className="text-xs text-red-500 mt-2 flex items-center gap-1">
            ...
            Below total minimums, we'll help you triage
          </p>
        )}
        {val >= totalMin && val > 0 && (
          <p className="text-xs text-emerald-600 mt-2 flex items-center gap-1">
            <CheckCircle2 className="w-3 h-3" />
            Covers all minimums, we'll optimise the rest
          </p>
        )}
      </div>
      ...
      <div className="flex gap-3 w-full">
        <button onClick={onBack}
          className="flex-1 py-3 border border-slate-200 rounded-2xl text-sm text-slate-600 hover:bg-slate-50 transition-colors">
          Back
        </button>
        <button
          onClick={() => val > 0 && onSubmit(val)}
          disabled={!budget || val <= 0}
          className="flex-1 py-3 text-white font-semibold rounded-2xl disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          style={{ background: val > 0 ? "#E8253A" : "#94a3b8" }}
        >
          Get advice
        </button>
      </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center">
        ...
        <h1 className="text-2xl font-bold text-slate-900 font-[var(--font-heading)] mb-2">
          Financial Autopilot
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
        ...
        <button
          ...
          className="group text-left bg-white border border-slate-100 rounded-2xl p-6 shadow-sm hover:border-amber-200 mode-card"
        >
          <div className="flex items-start justify-between mb-4">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center"
              style={{ background: "oklch(0.95 0.05 85)", border: "1px solid oklch(0.88 0.06 75)" }}>
          ...
          <p className="text-sm text-slate-500 leading-relaxed mb-4">
            Uses your registered budget of{" "}
            <strong className="text-slate-700">{fmt(USER_PROFILE.monthlyBudgetForDebt)}/mo</strong>.
            Get a full payment schedule using the avalanche method.
          </p>
          <div className="flex items-center gap-1.5 text-xs font-medium" style={{ color: "oklch(0.62 0.13 75)" }}>
            <TrendingDown className="w-3.5 h-3.5" />
            Minimise interest · fastest payoff
          </div>
        </button>
        ...
        <button
          ...
          className="group text-left bg-white border border-slate-100 rounded-2xl p-6 shadow-sm hover:border-red-200 mode-card"
        >
          <div className="flex items-start justify-between mb-4">
          ...
          <p className="text-sm text-slate-500 leading-relaxed mb-4">
            Tough month? Enter what you can actually afford and get specific
            advice on what to pay, what to skip safely.
          </p>
          <div className="flex items-center gap-1.5 text-xs font-medium text-red-500">
            <Flame className="w-3.5 h-3.5" />
            Damage control · avoid penalties
          </div>
        </button>
      </div>
    ...
    <div className="space-y-4">
      ...
      {mode === "select" && (
        <ModeSelect onSelect={m => {
          if (m === "planned") setMode("planned");
          else { setStressBudget(null); setMode("stress"); }
        }} />
      )}

      {mode === "planned" && (
        <PlannedResults onBack={() => setMode("select")} />
      )}

      {mode === "stress" && stressBudget === null && (
        <StressBudgetInput
          onSubmit={b => setStressBudget(b)}
